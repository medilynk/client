...
const Navbar = () => {
    ...
    return (
        <div>
        {!user && <nav className='w-full flex justify-between items-center bg-blue-700 px-24 fixed h-16 top-0 z-0'>
            <Link to='/' className="logo text-2xl text-white font-bold">
                <img src='/logo.png' alt='logo' className='w-8 h-8 inline-block mr-2' />
                Medilynk
            </Link>
            <div className="buttons flex gap-4">
                {!user && <Link to='/signin'><button className={buttonClasses}>Login</button></Link>}
